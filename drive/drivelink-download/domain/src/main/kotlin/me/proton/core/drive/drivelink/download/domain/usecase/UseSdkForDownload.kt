...
class UseSdkForDownload @Inject constructor(
    ...
    suspend operator fun invoke(driveLink: DriveLink) = coRunCatching {
        configurationProvider.preferSdkForDownload && when (driveLink.volumeType()) {
            null, Type.UNKNOWN -> false
            Type.REGULAR -> getFeatureFlag(driveAndroidSDKDownloadMain(driveLink.userId)).on
            Type.PHOTO -> getFeatureFlag(driveAndroidSDKDownloadPhoto(driveLink.userId)).on
        }
    }
