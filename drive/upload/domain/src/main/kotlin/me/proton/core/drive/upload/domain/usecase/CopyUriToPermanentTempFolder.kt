...
import me.proton.core.drive.base.domain.usecase.GetPermanentTempFolder
...
class CopyUriToPermanentTempFolder @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val name = uriResolver.getName(uriString) ?: UUID.randomUUID().toString()
        val lastModified = uriResolver.getLastModified(uriString)
        val destination = File(getPermanentTempFolder(userId, path), name)
            .apply {
                if (!exists()) {
                    createNewFile()
                } else {
