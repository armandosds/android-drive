...
@Entity(
    primaryKeys = [TYPE, URI],
    foreignKeys = [
        ForeignKey(
            entity = ScanResultInfoEntity::class,
            parentColumns = [ID],
            childColumns = [ID],
    ...
    indices = [
        Index(value = [ID, INDEX], unique = true)
    ]
)
data class ScanResultPageEntity(
    @ColumnInfo(name = ID)
    val id: Long,
    @ColumnInfo(name = TYPE)
    val type: ScannerOptions.OutputFormat,
    @ColumnInfo(name = URI)
